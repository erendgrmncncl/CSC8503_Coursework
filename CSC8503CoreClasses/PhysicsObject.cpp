...
void NCL::CSC8503::PhysicsObject::InitCapsuleInertia(){
	Vector3 dimensions = transform->GetScale();
	...
	float cylinderInertia = (12.0f * inverseMass) / (dimsSqr.x + dimsSqr.z);

	float sphereInertia = 2.5f * inverseMass / (radius * radius);

	inverseInertia.x = cylinderInertia;
	inverseInertia.y = sphereInertia;
	inverseInertia.z = cylinderInertia;
}
